param(
    ...
    [switch]$GitHubRegistrationRemoved
)
...
if (-not $GitHubRegistrationRemoved.IsPresent) {
    throw "Confirm that the exact ephemeral registration completed or was removed through GitHub before local cleanup."
}
...
if (-not (Test-Path -LiteralPath $BootstrapPath -PathType Leaf)) {
    throw "The exact runner cleanup target has no recovery identity."
}
...
if ($Bootstrap.schemaVersion -ne 2 -or
    ...
    [string]$Bootstrap.state -notin @("provisioning", "registration-attempted", "configured") -or
    $Bootstrap.ephemeral -ne $true) {
    throw "The exact runner cleanup target has an invalid recovery identity."
}
...
foreach ($RunnerProcess in $RunnerProcesses) {
    try {
        ...
        throw "Could not verify whether a runner listener is still using the cleanup target."
    }
    ...
    if ($ListenerPath.StartsWith($TargetPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "A runner listener is still using the exact cleanup target."
    }
}

Remove-Item -LiteralPath $TargetPath -Recurse -Force
if (Test-Path -LiteralPath $TargetPath) {
    throw "The exact runner root still exists after cleanup."
}
Write-Output "Removed the verified local runner root for $Wave/$Variant."
