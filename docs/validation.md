...
- The self-hosted job passes `GITHUB_SHA` to `scripts/package-plugin.ps1`, verifies the packaged `SourceManifest.json`, `Web/dist/index.html`, and exact committed `LICENSE` blob, then generates a fresh non-secret marker and creates a temporary host project with a project-only `Config/UnrealEditorWebUI/ToolCatalog.json` fixture. Native `UnrealEditorWebUI.Bridge.ProjectToolCatalog` reads that fixture through the production no-argument getter, and `UnrealEditorWebUI.Bridge.WebUIHealth` locks the health response and redaction contract. The job also runs the remaining headless Bridge/Settings automation filters, executes the packaged bridge and settings smokes, and then runs `UnrealEditorWebUI.Browser.CEFBindingAndTaskEvent` in the real GUI editor. The GUI test observes the same fresh marker only through the rendered React project/stage/category controls and catalog status, opens the product health panel, generates the closed schema-v2 report, and proves the marker, host-project identity, page location, task id, nonce, payload, response, and logs are absent before completing the existing task/event/TaskCard round trip. Its expected Tool Pack aggregate is exported from the actual host fixture array (`0` for UE 5.3 and `3` for UE 5.8) and rejected by native code unless it is a canonical bounded integer. Because the marker is generated after `BuildPlugin` and the fixture remains outside the packaged plugin, this proves project configuration reaches the packaged UI without rebuilding `Web/dist`. The job checks every required Automation path, a fresh structured packaged-smoke result, and explicit success markers for both Python smokes rather than trusting process exit codes alone. `SourceManifest.buildToolchain`, written by the exact-commit frontend staging process, is the sole source for the Node/npm versions later claimed as the build toolchain; the earlier prerequisite `npm --version` is only an availability check.
