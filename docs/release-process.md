...
The commandlet smoke does not create an interactive CEF window. The separate `UnrealEditorWebUI.Browser.CEFBindingAndTaskEvent` test runs through `UnrealEditor.exe` on the required interactive `gui` runner and verifies the packaged React page, delayed JavaScript binding, native health context, the rendered support-report allowlist and negative privacy canaries, `system.ping` task execution, native task event, DOM delivery, task detail, and rendered React task card. The workflow writes `UE_WEBUI_EXPECTED_TOOL_PACK_COUNT` from the actual temporary-host Tool Pack source array: currently `0` for UE 5.3 and `2` for UE 5.8. The native test rejects a missing, non-canonical, negative, or out-of-range value before injecting it as the CEF aggregate-count expectation; a direct manual invocation must set the same environment variable for its host fixtures. Release provenance requires both validation layers from the same exact-commit UE 5.8 job.
